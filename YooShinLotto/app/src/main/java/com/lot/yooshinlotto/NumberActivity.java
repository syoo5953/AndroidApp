...
public class NumberActivity extends AppCompatActivity implements GoogleApiClient.OnConnectionFailedListener {
        ...
        try {
            ...
            repickBtn.setOnClickListener(v -> {
                for (int i = 0; i < lotto_numbers.length; i++) {
                    ...
                    lotto_numbers[i] = getRandomNums(1, 45, most_occurred_nums);
                    for (int j = 0; j < i; j++) {
                ...
                Arrays.sort(lotto_numbers);
                for(int i = 0; i < lotto_numbers.length; i++) {
                    System.out.println("most_occured = " + most_occurred_nums[i] + " lotto_numbers = " + lotto_numbers[i]);
                }
 ...
 */
    ...
    public int getRandomNums(int min , int max, int[] most_occurred_nums) {
        Random random = new Random();
        int p = random.nextInt(10);
        System.out.println("p = " + p);
        if (p < 2) {
            return most_occurred_nums[random.nextInt(most_occurred_nums.length)];
        } else {
